/*
  ...
  NOTE:  Addresses below are only valid when IOCON.BANK=0 (register addressing
  mode)
         This means one of the control register values can change register
  addresses!
         The default values is 0, so that's how we're using it.

         All registers except ICON (0xA and 0xB) are paired as A/B for each
  8-bit GPIO port.
         Comments identify the port's name, and notes on how it is used.

         *THIS CLASS ENABLES THE ADDRESS PINS ON ALL CHIPS ON THE BUS WHEN THE
  FIRST CHIP OBJECT IS INSTANTIATED!

  USAGE: All Read/Write functions except wordWrite are implemented in two
  different ways.
         Individual pin values are set by referencing "pin #" and On/Off,
  Input/Output or High/Low where
         portA represents pins 0-7 and portB 8-15. So to set the most
  significant bit of portB, set pin # 15.
         To Read/Write the values for the entire chip at once, a word mode is
  supported buy passing a
         single argument to the function as 0x(portB)(portA). I/O mode Output is
  represented by 0.
         The wordWrite function was to be used internally, but was made public
  for advanced users to have
         direct and more efficient control by writing a value to a specific
  register pair.

  VERSION RELESE NOTES:
        V0.2
        Changed direct manipulation of pin 10 on ATMega168/328 via "PORTB" to
  use digitalWrite on an arbitrary
...
#define CLOCK_DIVIDER                                                          \
  (2) // SPI bus speed to be 1/2 of the processor clock speed - 8MHz on most
      // Arduinos

// Control byte and configuration register information - Control Byte: "0100 A2
// A1 A0 R/W" -- W=0

#define OPCODEW                                                                \
  (0b01000000) // Opcode for MCP23S17 with LSB (bit0) set to write (0), address
               // OR'd in later, bits 1-3
#define OPCODER                                                                \
  (0b01000001) // Opcode for MCP23S17 with LSB (bit0) set to read (1), address
               // OR'd in later, bits 1-3
#define ADDR_ENABLE                                                            \
  (0b00001000) // Configuration register for MCP23S17, the only thing we change
               // is enabling hardware addressing
...
MCP::MCP(uint8_t address, uint8_t ss)
{
  ...
  _invertCache =
      0x0000; // Default input inversion state is not inverted, 0x0000
};
...
void MCP::begin()
{
  ...
  // SPI.setBitOrder(MSBFIRST);          // Sets SPI bus bit order (this is the
  // default, setting it for good form!)
  // SPI.setDataMode(SPI_MODE0);         // Sets the SPI bus timing mode (this
  // is the default, setting it for good form!)
  byteWrite(IOCON, ADDR_ENABLE);
}

// GENERIC BYTE WRITE - will write a byte to a register, arguments are register
...
{                           // Accept the register and byte
  ...
  SPI.transfer(
      OPCODEW |
      (_address << 1)); // Send the MCP23S17 opcode, chip address, and write bit
...
{                           // Accept the start register and word
  ...
  SPI.transfer(
      ...
      (_address << 1)); // Send the MCP23S17 opcode, chip address, and write bit
  SPI.transfer(reg);    // Send the register we want to write
  SPI.transfer((uint8_t)(word)); // Send the low byte (register address pointer
                                 // will auto-increment after write)
  SPI.transfer((uint8_t)(
      word >> 8)); // Shift the high byte down to the low byte location and send
  ::digitalWrite(_ss, HIGH); // Take slave-select high
}
...
{ // Accept the pin # and I/O mode
  if (pin<1 | pin> 16)
    return; // If the pin value is not valid (1-16) return, do nothing and
            // return
  if (mode == INPUT)
  { // Determine the mode before changing the bit state in the mode cache
    _modeCache |=
        1 << (pin -
              1); // Since input = "HIGH", OR in a 1 in the appropriate place
  }
  else
  {
    _modeCache &= ~(1 << (pin - 1)); // If not, the mode must be output, so and
                                     // in a 0 in the appropriate place
  }
  wordWrite(IODIRA, _modeCache); // Call the generic word writer with start
                                 // register and the mode cache
}

void MCP::pinMode(unsigned int mode)
{                           // Accept the word…
  wordWrite(IODIRA, ~mode); // Call the the generic word writer with start
                            // register and the mode cache
  _modeCache = ~mode;
}

// THE FOLLOWING WRITE FUNCTIONS ARE NEARLY IDENTICAL TO THE FIRST AND ARE NOT
...
{ // This function will read all 16 bits of I/O, and return them as a word in
  // the format 0x(portB)(portA)
  unsigned int value =
      0; // Initialize a variable to hold the read values to be returned
  ::digitalWrite(_ss, LOW); // Take slave-select low
  SPI.transfer(
      OPCODER |
      (_address << 1)); // Send the MCP23S17 opcode, chip address, and read bit
  SPI.transfer(GPIOA);  // Send the register we want to read
  value = SPI.transfer(0x00); // Send any byte, the function will return the
                              // read value (register address pointer will
                              // auto-increment after write)
  value |= (SPI.transfer(0x00) << 8); // Read in the "high byte" (portB) and
                                      // shift it up to the high location and
                                      // merge with the "low byte"
  ::digitalWrite(_ss, HIGH);          // Take slave-select high
...
{ // This function will read a single register, and return it
  ...
  value = SPI.transfer(
      0x00); // Send any byte, the function will return the read value
  ::digitalWrite(_ss, HIGH); // Take slave-select high
...
{ // Return a single bit value, supply the necessary bit (1-16)
  if (pin<1 | pin> 16)
    return 0x0; // If the pin value is not valid (1-16) return, do nothing and
                // return
  return digitalRead() & (1 << (pin - 1)) ? HIGH : LOW; // Call the word reading
                                                        // function, extract
